...
test.node.num = function(){
  ...
  checkEquals(cypher(buildDataBase(),nodenum.query1)$c,5)
  
  nodenum.query2 = "match (n:AssetCategory) return count(n) AS c"
  checkEquals(cypher(buildDataBase(),nodenum.query2)$c,7)
}
...
test.rel.num =function(){
  ...
  checkEquals(cypher(buildDataBase(),relnum.query1)$c,5)
  
  relnum.query2 = "match (a)-[r:IS_ELIGIBLE_UNDER]->(b) return count(r) as c"
  checkEquals(cypher(buildDataBase(),relnum.query2)$c,24)
  
  relnum.query3 = "match (a)-[r:IS_AVAILABLE_FOR]->(b) return count(r) as c"
  checkEquals(cypher(buildDataBase(),relnum.query3)$c,18)
  
  relnum.query4 = "match (a)-[r:POSSESSES]->(b) return count(r) as c"
  checkEquals(cypher(buildDataBase(),relnum.query4)$c,9)
  
  relnum.query5 = "match (a)-[r:ACCESSES]->(b) return count(r) as c"
  checkEquals(cypher(buildDataBase(),relnum.query5)$c,27)
}
...
test.rel.type = function(){
 ...
 # rel.query = list()
  ...
  checkEquals(cypher(buildDataBase(),rel.query1)$rel1, 'IS_ELIGIBLE_UNDER')
...
test.rel.prop = function(){
  ...
  checkEquals(cypher(buildDataBase(),prop.query1)$q, 20000)
  prop.query2 = "match (c:Client {id:'c2'})-[p:POSSESSES]->(a:Asset) where a.ISIN='US912796HW25' return p.quantities as q"
  checkEquals(cypher(buildDataBase(),prop.query2)$q, 100000)
